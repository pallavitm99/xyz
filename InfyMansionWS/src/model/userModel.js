...
userModel.findUser = (emailId)=> {   
    return dbModel.getUserCollection().then( model => {  
        ...
        console.log("In collection",emailId);   
...
userModel.findProperty=()=>{
    return dbModel.getPropertyCollection().then( model=>{
        return model.find().then( propertyData=>{
...
userModel.viewProperty=( propertyId )=>{
    return dbModel.getPropertyCollection().then( model=>{
        return model.findOne( {"propertyId": propertyId} ).then( propertyData=>{
            if( propertyData ) return propertyData
            else return null
        } )
    } )
...
userModel.getallUsers=()=>{
    ...
    return dbModel.getUserCollection().then( model=>{
        return model.find().then( userData=>{
            if( userData ){
                return userData
            } else{
                return null
...
userModel.updateP=( name,contactNo,userId )=>{
    return dbModel.getUserCollection().then( model=>{       
        return model.updateOne( {"userId": userId},{$set: {'name': name,'contactNo': contactNo}} ).then( updated=>{
